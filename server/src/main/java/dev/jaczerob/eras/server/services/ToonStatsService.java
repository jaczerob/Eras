...
import dev.jaczerob.eras.server.models.toonstats.Toon;
import dev.jaczerob.eras.server.models.toonstats.ToonStats;
import dev.jaczerob.eras.server.repositories.ToonRepository;
...
public class ToonStatsService {
    ...
    public ToonStatsService(final ToonRepository toonRepository) {
        this.toonRepository = toonRepository;
    }

    public ToonStats getToonStats() {
        final Iterable<ToonEntity> toonEntities = toonRepository.findAll();
        final List<Toon> toons = new ArrayList<>();
        toonEntities.iterator().forEachRemaining(toonEntity -> toons.add(Toon.fromEntity(toonEntity)));
